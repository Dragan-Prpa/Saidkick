...
class MyToolWindowFactory : ToolWindowFactory {
    ...
    class MyToolWindow(private val project: Project) {
        ...
        private val mcpServerManager = project.service<McpServerManager>()
        private val assistant = SaidkickAssistant(config, mcpServerManager)
        ...
        private val refreshMcpButton = JButton("Refresh MCP")
        private val mcpStatusLabel = JLabel("MCP: initializing...")
        ...
        private val content = JBPanel<JBPanel<*>>(BorderLayout()).apply {
            ...
            val north = JPanel(BorderLayout()).apply {
                add(mcpStatusLabel, BorderLayout.CENTER)
                add(refreshMcpButton, BorderLayout.EAST)
            }
        ...
        init {
            ...
            refreshMcpButton.addActionListener { refreshMcpStatus(notify = true) }
        }
        ...
        private fun refreshMcpStatus(notify: Boolean) {
            ApplicationManager.getApplication().executeOnPooledThread {
                val snapshots = runCatching { mcpServerManager.refreshAll() }
                    .getOrElse { error ->
                        ApplicationManager.getApplication().invokeLater {
                            mcpStatusLabel.text = "MCP: unavailable (${error.message ?: "unknown error"})"
                            if (notify) {
                                SaidkickNotifications.warning(
                                    project,
                                    "MCP",
                                    "Failed to refresh MCP servers: ${error.message ?: "unknown error"}",
                                )
                            }
                        }
                ...
                val statusText = "MCP: ${mcpServerManager.describeStatus()}"
                ApplicationManager.getApplication().invokeLater {
                    mcpStatusLabel.text = statusText
                    if (notify) {
                        val connectedCount = snapshots.count { it.state == McpConnectionState.CONNECTED }
                        val errors = snapshots.filter { it.state == McpConnectionState.ERROR }
                        when {
                            connectedCount > 0 -> {
                                SaidkickNotifications.info(
                                    project,
                                    "MCP",
                                    "Connected to $connectedCount server(s).",
                                )
                            }

                            errors.isNotEmpty() -> {
                                val firstError = errors.first().lastError ?: "unknown error"
                                SaidkickNotifications.warning(
                                    project,
                                    "MCP",
                                    "No active MCP connections. First error: $firstError",
                                )
                            }
                        }
                    }

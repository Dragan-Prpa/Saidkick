...
On startup, if identity values are fully configured, Saidkick greets the developer. If the developer is inactive or change bursts are detected, Saidkick sends proactive messages generated by the LLM from internal prompts (for example, asking if help is needed or reminding to commit).

Proactive LLM responses are constrained to exactly one sentence and at most 20 words.
